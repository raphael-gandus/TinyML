...
void setup() {
  ...
  //Questions
  Serial.println("__________Questions__________");
  Serial.println("1) Vous vous appelez Raphaël ?");
  Serial.println();
  Serial.println("2) L'année actuelle est 2035 ?");
  Serial.println();
  Serial.println("3) TinyML est-il un outil important pour le machine learning appliqué aux objets connéctés ?");
  Serial.println();
  Serial.println("Veuillez répondra aux questions une par une en effectuant un geste (yes ou no) avec la main");
  Serial.println("(Attention pour le signe no : ne pas faire une croix mais deux lignes horizontales de gauche à droite puis de droite à gauche)");
  Serial.println();
  Serial.println();
  Serial.println("__________Réponses__________");
...
void loop() {
  ...
  for (int i = 0; i < NUM_GESTURES; i++) 
  {
    ...
    {
      Serial.print("Votre réponse à la question est : ");
